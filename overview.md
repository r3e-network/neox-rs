...
主要开放项：RPC 模拟与 Geth SkipTransactionChecks 语义差异、TPKE commitment 绑定的跨实现向量、Beacon/dbft 混合互通、DKG epoch/recovery 活体门禁、Reth 最新 tip 的独立同步演练。Windows blst 编译遇到 target 时间戳错误，目标测试未记为通过。
