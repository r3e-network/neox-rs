...
1. **Sync/import:** the first fresh-datadir phase is recorded in
   ...
   where the original `4.214x` comparison remains invalid because the Geth timer included an
   asymmetric 270.8-second pre-trigger wait. The corrected replacement uses six fresh-datadir runs,
   two sync-source implementations, one shared trigger barrier per run, complete event/workload
   metadata, final hash/root gates, and offline restart validation. On the narrow block 1–20,000
   workload, `neox-rs` finished first in all six runs with a paired ratio median of 1.455x and an
   observed range of 1.051x–1.515x; the report explicitly does not generalize this transaction-sparse
   sample to overall full-node performance.
